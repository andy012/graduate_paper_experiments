...
/**
  * Created by andy on 17/02/2017.
  */

case class RandomTextWriterConfig(output: String = "hdfs://192.168.3.105:9000/video/tout.txt",
                                  minKey: Int = 5, maxKey: Int = 10,
                                  minValue: Int = 10, maxValue: Int = 100,
                                  mapNum: Int = 1,
                                  userName: String = "spark",
                                  megaBytesPerMap: Long = 1024)
...
object RandomTextWriter {
  ...
  def main(args: Array[String]): Unit = {
    ...
    parser.parse(args, RandomTextWriterConfig()) map { config =>
      ...
      val sparkConf = new SparkConf().setMaster("local[4]").setAppName("WriteData")
